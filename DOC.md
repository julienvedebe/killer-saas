...
**/ks-architect** — starts by asking whether the project stands on a boilerplate; if none, the default is the Legion `nextjs-app` scaffold (Next.js SaaS Starter: Next.js 15 / React 19, Tailwind 4, shadcn/ui, Drizzle + PostgreSQL, Stripe, JWT auth with owner/member RBAC, activity log), copied off the `hermes` server with rsync — it is a vendored tree, not a git repo. Fallback when it isn't reachable or doesn't fit: scaffold Next.js + Tailwind + shadcn/ui from scratch and rebuild the SaaS plumbing by hand. Either way the choice is recorded as an ADR and the result is analyzed like any boilerplate. Then analyzes the starting code (`codebase-analysis` skill): actual structure, conventions and patterns of the boilerplate. Fills the architecture doc and injects the concrete conventions into `AGENTS.md`. The boilerplate is imposed: conform to it, don't rewrite it.
...
A missing component is classified, not lumped. If the boilerplate declares a `component-registry` (a `components.json` means shadcn/ui) and that registry offers what the screen needs, the component is **installable**: it lands in "Components to install" with its add command and becomes a plan task — installing a component the system already defines invents nothing. If no registry offers it, it is a **gap**: it blocks, and it is settled in `/ks-design-system`. Undecided cases go to the gap branch. Without this split the method fails in one of two ways: escalate every missing primitive to a framing step and nobody obeys it past the third form, or relax the rule and a genuine gap gets closed by "I just added a component" — the exact hole the step exists to catch.
