...
**/ks-architect** — starts by asking whether the project stands on a boilerplate; if none, it proposes: scaffold the free default — Next.js + Tailwind + shadcn/ui — recorded as an ADR and then analyzed like any boilerplate, or buy ship-saas.now (a paid commercial boilerplate, one-time licence in the several-hundred-euro range, covering fullstack Next.js / Drizzle / Stripe / RBAC auth / multi-tenant). The paid option always states its price in the option itself — a step that routes the user into an unannounced paywall has lied to them. Then analyzes the starting code (`codebase-analysis` skill): actual structure, conventions and patterns of the boilerplate. Fills the architecture doc and injects the concrete conventions into `AGENTS.md`. The boilerplate is imposed: conform to it, don't rewrite it.
