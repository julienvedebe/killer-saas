...
- `/ks-design-system`  binds the Open Design design system + mirrors it (docs/design-system.md)
- `/ks-research`   explores the story's real context (current code, APIs, traps)
- `/ks-design`     generates a story's screen in Open Design, from that system (UI stories)
...
## Design
The design phase runs on **Open Design**. The global design system lives in `docs/design-system.md`: a versioned mirror of the Open Design system bound to this project, plus the boilerplate's real component inventory. Each story's design lives in `docs/designs/<id>.md` (+ its `-brief.md` prompt and a reference `.html` mockup).
- `docs/design-system.md` carries the binding in its frontmatter (`open-design: project / design-system / skill`). `/ks-design` is fail-closed on it: no binding, no design.
- Screens are **generated by Open Design**, never drawn by the agent and never hand-edited afterwards. A failed run produces no design file; a wrong screen is refined (`/ks-design <id> --refine "…"`), not patched.
